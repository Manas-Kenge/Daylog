...
use crate::ui::{
    format_duration,
    overview::{
        panel_title, render_top_apps_panel, render_top_categories_panel,
...
fn build_columns(
    ...
    let total_days = (today - week_start).num_days() + 1;
    ...
    for c in 0..num_cols {
        ...
        let mut cells: [Option<HeatmapCell>; 7] =
...
fn render_this_month(f: &mut Frame, area: Rect, app: &App) {
    ...
    let lines = vec![
        row("Total", format_duration(stats.total_secs)),
        row("Average", format_duration(stats.avg_secs)),
        row("Best", format_duration(stats.best_secs)),
        row("Streak", format!("{} days", stats.streak)),
    ];
    let p = Paragraph::new(lines);
    f.render_widget(p, inner);
}
...
fn month_stats(today_active: f64, trailing: &[f64]) -> MonthStats {
    ...
    let best = days.iter().fold(0.0_f64, |a, &b| a.max(b));
...
mod tests {
    ...
    fn build_columns_covers_full_year_window() {
        ...
        // 53 columns is the steady state; depending on the start
        // weekday a 54th column slips in. Anything else is a bug.
        assert!(
            cols.len() == 53 || cols.len() == 54,
            "expected 53–54 columns, got {}",
            cols.len()
        );
        // Today's cell must be flagged in the last column at today's
        // weekday row.
        let today_dow = today.weekday().num_days_from_sunday() as usize;
        let last = cols.last().unwrap();
        assert!(
            last.cells[today_dow]
                .as_ref()
                .is_some_and(|c| c.is_today),
            "today cell missing from last column"
        );
    }
    ...
    fn build_columns_emits_at_least_twelve_month_labels() {
        ...
        let labels: Vec<&'static str> =
            cols.iter().filter_map(|c| c.month_label).collect();
    ...
    fn month_stats_averages_only_over_active_days() {
        ...
        assert_eq!(stats.best_secs, 4.0 * 3600.0);
