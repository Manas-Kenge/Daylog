...
A single-window native app that shows a beautiful real-time view of your day, sourced from a local [ActivityWatch](https://activitywatch.net) server. No browser tab, no cloud, no sign-in.

## What it isn't

- A replacement for `aw-server` or `aw-awatcher`. Daylog only renders the UI.
- Cross-platform (Linux-first; macOS/Windows are not on the roadmap for v0.1).
- A behavioral nudging tool. Daylog is observational.

## Prerequisites

- Linux with WebKitGTK 4.1 (Ubuntu 22.04+, Fedora 39+, Debian 12+, or any rolling-release distro).
- That's it. Daylog bundles its own copy of [`aw-server-rust`](https://github.com/ActivityWatch/aw-server-rust) and [`awatcher`](https://github.com/2e3s/awatcher) — install one artifact and you have everything. If you already have ActivityWatch installed, Daylog detects it and uses your existing setup instead.

## Supported Linux distros

Daylog ships three artifacts that together cover virtually every active desktop Linux distro:
