 ...
 *
 * Two halves:
 *   Top: focus ring (mins-on-current-app vs 60-min goal) + app/title +
 *        small stats list (started · switches · category).
 *   Bottom: AFK ring (active% with raw seconds), three stacked rows
 *        (active / afk / tracked), and a horizontal ratio bar.
 */
        ...
        <>
          <div className="flex items-center gap-3">
            ...
            <div className="flex min-w-0 flex-col gap-0.5">
              <div className="truncate font-mono tabular-nums text-base font-semibold tracking-tight">
                {session?.app || "—"}
              </div>
          ...
          <div className="mt-3 flex flex-col">
            <Stat
              ...
              v={
                session ? (
                  <span className="font-mono tabular-nums">{fmtClock(session.start)}</span>
                ) : (
                  "—"
                )
              }
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-1">
              <div className="flex justify-between">
                ...
                <span className="font-mono tabular-nums font-medium">
                  {fmtDuration(afk?.active_seconds ?? 0)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">afk</span>
                <span className="font-mono tabular-nums font-medium">
                  {fmtDuration(afk?.afk_seconds ?? 0)}
                </span>
              </div>
              <div className="flex justify-between">
